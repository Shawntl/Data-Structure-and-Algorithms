...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> TreeNode:
        ...
        def recursiveBuildTree(preorder_left, preorder_right, inorder_left, inorder_right):
            '''
            preorder_left: 当前子树前序遍历在全树中起始位置
            preorder_right: 当前子树前序遍历在全树中终止位置
            inorder_left: 当前子树中序遍历它的左子树左边界
            inorder_right: 当前子树中序遍历它的右子树右边界
            '''
            # 当前序列中没有值返回
            ...
            # 左子树长度
            leftTree_num = root_inorder_index - inorder_left
            # 构建根节点, 根节点就是按前序遍历的顺序
            node = TreeNode(root)

            # 向下递归构建左右子树
            node.left = recursiveBuildTree(preorder_left+1, 
                                           preorder_left+leftTree_num, 
                                           inorder_left,
                                           root_inorder_index-1)
            node.right = recursiveBuildTree(preorder_left+leftTree_num+1,
                                            preorder_right,
                                            root_inorder_index+1,
                                            inorder_right)
            return node
        root = recursiveBuildTree(0, len(preorder)-1, 0, len(inorder)-1)
        return root
```
